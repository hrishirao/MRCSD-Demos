...
def Img2Screen (img):
    ...
    while(counter<len(img)+top_buffer):
        row_frame = image_row_frame + bytearray([chr(counter+1-top_buffer)])
        for x in range (0,side_buffer):
...
def AnnounceContent(row,column):
    point_height_frame = point_status_frame + bytearray(chr(row)) + bytearray(chr(column))
    ...
    if(row<=15 and column<=15):
                if(list(height_response)[2]==chr(row) and list(height_response)[3]==chr(column)):
                    ...
                    point_notify = point_update_frame +bytearray([row,column,4,0x04])
                    UpdateScreen(point_notify)
                    time.sleep(1)
                    point_notify = point_update_frame +bytearray([row,column,height_response,0x00])
                    UpdateScreen(point_notify)
                    time.sleep(0.5)
    print "!!!"
