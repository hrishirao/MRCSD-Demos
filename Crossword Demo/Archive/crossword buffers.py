...
def GetLocation(response):
    ...
    return(row-top_buffer,column-side_buffer)
...
def AnnounceContent(row,column):
    ...
    if(row<=15 and column<=15):
                ...
                if(list(height_response)[2]==chr(row+top_buffer) and list(height_response)[3]==chr(column+side_buffer)):
                    ...
                    point_notify = point_update_frame +bytearray([row+top_buffer,column+side_buffer,4,0x04])
                    UpdateScreen(point_notify)
                    time.sleep(1)
                    point_notify = point_update_frame +bytearray([row+top_buffer,column+side_buffer,height_response,0x00])
                    UpdateScreen(point_notify)
                    time.sleep(0.5)
    print "!!!"
